...
public class Day {
	...
	public static final String[] wd = {"日", "月", "火", "水", "木", "金", "土"};
	...
	//----- その月の大小歴を求める -----//
	public static int maxDate(int year, int month) {
		//大小歴を格納する変数を用意
		int maxDate = 0;
		//閏年の場合
		if (isLeap(year)) {
			//閏年の大小歴配列から、該当月の大小歴を参照
			maxDate = dateLeapLS[month - 1];
		}
		//平年の場合
		else {
			//平年の大小歴配列から、該当付きの大小歴を参照
			maxDate = dateLS[month - 1];
		}
		//返却値
		return maxDate;
	}
