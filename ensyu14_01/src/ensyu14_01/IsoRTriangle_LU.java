...
* 	@see Triangle
*/
public class IsoRTriangle_LU extends Triangle {
	 ...
	 */
	public IsoRTriangle_LU(int length) {
		super(length, length);
	}
	 ...
	 * 返却値：	String型、「IsoRTriangle_LU(length:3)」※3は長さに応じた値<BR>
	 * @return	"IsoRTriangle_LU(length:" + length + ")"
	 */
	@Override
	public String toString() {
		return "IsoRTriangle_LU(length:" + getWidth() + ")";
	}
	//----- メソッド：図形を描画する -----//
	/**
	...
	public void draw() {
		...
		for (int vertical = 1; vertical <= getWidth(); vertical++) {
			//横ループ（アスタリスク）
			for (int beside = 1; beside <= (getWidth()-vertical+1); beside++) {
				//アスタリスクを表示
				System.out.print('*');
				//ただし2文字以上並ぶとき
				if (beside < (getWidth() - vertical + 1))
