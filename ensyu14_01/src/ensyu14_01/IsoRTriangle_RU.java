...
* 	@see Triangle
*/
public class IsoRTriangle_RU extends Triangle {
	 ...
	 */
	public IsoRTriangle_RU(int length) {
		super(length, length);
	}
	 ...
	 * 返却値：	String型、「IsoRTriangle_RU(length:3)」※3は長さに応じた値<BR>
	 * @return	"IsoRTriangle_RU(length:" + length + ")"
	 */
	@Override
	public String toString() {
		return "IsoRTriangle_RU(length:" + getWidth() + ")";
	}
	//----- メソッド：図形を描画する -----//
	/**
	...
	public void draw() {
		...
		for (int vertical = 1; vertical <= getWidth(); vertical++) {
			...
			//横ループ（アスタリスク）
			for (int besideS = 1; besideS <= (getWidth() -vertical + 1); besideS++) {
				//アスタリスクを表示
				System.out.print('*');
				//ただし2文字以上並ぶとき
				if (vertical < getWidth())
