...
//===================== 具体クラス：右上直角二等辺三角形を表す =====================
/**
* 概要：	右上直角二等辺三角形を表すクラス。<BR>
* 補足：<BR>
* 			このクラスは、直角二等辺三角形を表す抽象クラスAbstRiIsoTriangleから派生したクラス。<BR>
* 	@see Shape
* 	@see AbstRiIsoTriangle
*/
public class RiIsoTriangle_RU extends AbstRiIsoTriangle {
	 ...
	 */
	public RiIsoTriangle_RU(int length) {
		super(length);
	}

	//----- メソッド：図形情報を表す文字列を返却する -----//
	/**
	 * 概要：	右上直角二等辺三角形を表す文字列を返却するメソッド。<BR>
	 * 返却値：	String型、「RiIsoTriangle_RU(length:3)」※3は長さに応じた値<BR>
	 * @return	"RiIsoTriangle_RU(length:" + getLength() + ")"
	 */
	@Override
	...
	public void draw() {
		...
		for (int vertical = 1; vertical <= getLength(); vertical++) {
			...
			//横ループ（アスタリスク）
			for (int besideS = 1; besideS <= (getLength() -vertical + 1); besideS++) {
				//アスタリスクを表示
				System.out.print('*');
				//ただし2文字以上並ぶとき
				if (vertical < getLength())
