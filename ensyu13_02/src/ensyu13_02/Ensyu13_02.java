	...
	public static void main(String[] args) {
		...
		do {
			...
			for (int nItr = 0; nItr < artN; nItr++) {
				...
				int selectShapeN = 0;
				//1 <= selectShapeN <= 5 となるまで聞き直すかたまり
				do {
					//ヘッダと、選択できる図形の種類を表示
					System.out.print((nItr + 1) + "番の図形の種類（1…点／2…水平直線／3…垂直直線／4…長方形／5…直角二等辺三角形）：");
					//入力値をselectShapeNとして扱う
					selectShapeN = stdIn.nextInt();
					//1～5以外が入力された場合のエラーメッセージ
					if (selectShapeN < 1 || selectShapeN > 5) {
						System.out.println("1～5を選択してください。");
					}
				//1～4以外が入力されている間はループ
				} while(selectShapeN < 1 || selectShapeN > 5);
				...
				case 5: {
					...
					case 1: {
						...
						shape[nItr] = new RiIsoTriangle_LD(longInt("辺の長さ"));
						//ここでswitchを抜ける
						break;
					}
					//2…左上
					case 2: {
						...
						shape[nItr] = new RiIsoTriangle_LU(longInt("辺の長さ"));
						//ここでswitchを抜ける
						break;
					}
					//3…右下
					case 3: {
						...
						shape[nItr] = new RiIsoTriangle_RD(longInt("辺の長さ"));
						//ここでswitchを抜ける
						break;
					}
					//4…右上
					case 4: {
						...
						shape[nItr] = new RiIsoTriangle_RU(longInt("辺の長さ"));
						//ここでswitchを抜ける
						break;
					}
					}
