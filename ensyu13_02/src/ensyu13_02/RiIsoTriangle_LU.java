...
*/
public class RiIsoTriangle_LU  extends AbstRiIsoTriangle {
	 ...
	 */
	public RiIsoTriangle_LU(int length) {
		super(length);
	}

	//----- メソッド：図形情報を表す文字列を返却する -----//
	/**
	 * 概要：	左上直角二等辺三角形を表す文字列を返却するメソッド。<BR>
	 * 返却値：	String型、「RiIsoTriangle_LU(length:3)」※3は長さに応じた値<BR>
	 * @return	"RiIsoTriangle_LU(length:" + getLength() + ")"
	 */
	@Override
	...
	public void draw() {
		...
		for (int vertical = 1; vertical <= getLength(); vertical++) {
			//横ループ（アスタリスク）
			for (int beside = 1; beside <= (getLength()-vertical+1); beside++) {
				//アスタリスクを表示
				System.out.print('*');
				//ただし2文字以上並ぶとき
				if (beside < (getLength() - vertical + 1))
